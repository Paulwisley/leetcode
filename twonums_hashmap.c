...
HashMap* hash_create(int size){
    ...
    hashMap->size = size;
    hashMap->storage = calloc(size, sizeof(HashNode*));//根据size的大小 创建hash头节点链
    return hashMap;
}
...
void hash_set(HashMap *hashMap, int key, int value) { //设置hash 根据定义的哈希函数 将对应的结点加到对应的链中去
    int hash = abs(key) % hashMap->size; //取模
    HashNode* node;
